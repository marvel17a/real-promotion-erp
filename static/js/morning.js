// Handles: fetching prior stock, adding/removing rows, totals, and arrow-key navigation.
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchAndPopulate() {
    ...
    if (!employeeId || !dateStr) {
      ui.tableBody.innerHTML =
        '<tr><td colspan="8" class="text-muted p-4">Select an employee and date to begin.</td></tr>';
      return;
    }
  ...
  function createRow(productData = {}) {
    ...
    tr.innerHTML = `
      ...
      <td><button type="button" class="btn-remove-row btn btn-sm btn-danger" title="Remove row">Delete</button></td>
    `;
    const select = tr.querySelector(".product-dropdown");
    if (productData.id) select.value = productData.id;
  ...
  function handleArrowKeyNavigation(e) {
    ...
    if (
      !activeElement ||
      !ui.tableBody.contains(activeElement) ||
    ...
    if (key === "ArrowUp" || key === "ArrowDown") {
      ...
      if (nextRowIndex >= 0 && nextRowIndex < rows.length) {
        ...
        const activeElementClass = activeElement.className
          .split(" ")
          .find((c) => ["product-dropdown", "given", "price"].includes(c));
  ...
  ui.tableBody.addEventListener("input", (e) => {
    if (e.target.matches(".given, .price")) recalculateTotals();
  });
