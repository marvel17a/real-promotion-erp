document.addEventListener("DOMContentLoaded", () => {
    ...
    /**
     * Fetches remaining stock and populates the table automatically.
     */
    async function fetchAndPopulate() {
        const employeeId = ui.employeeSelect.value;
        const dateStr = ui.dateInput.value;

        ui.tableBody.innerHTML = ''; // Clear the table
        ...
        try {
            ...
            if (Object.keys(data).length === 0) {
                // Scenario B: New Employee or no remaining stock
                ui.fetchMsg.textContent = "No remaining stock found. Starting a new allocation.";
                createRow(); // Add one blank row to start
            } else {
                // Scenario A: Existing Employee with stock
            ...
            createRow(); // Add a blank row even if there's an error
        } finally {
            recalculateTotals();
        }
    }
    ...
    function recalculateTotals() {
        ...
        ui.tableBody.querySelectorAll("tr").forEach(row => {
            ...
            if (!row.querySelector(".total")) return; // Skip non-data rows
    ...
    /**
     * Handles keyboard navigation within the table using arrow keys.
     */
    function handleArrowKeyNavigation(e) {
        ...
        // Ensure we are inside the table and on a focusable element
        ...
        if (key === 'ArrowUp' || key === 'ArrowDown') {
            ...
            if (nextRowIndex >= 0 && nextRowIndex < rows.length) {
                ...
                // Try to find an element in the same column by its class
        ...
        if (targetElement) {
            targetElement.focus();
            // For input fields, select the content to make it easy to overwrite
