        ...
        try:
            ...
            for i, pid in enumerate(p_ids):
                ...
                if gv_qty > 0:
                    ...
                    prod = cursor.fetchone()
                    if prod and int(prod['stock']) < gv_qty:
                        flash(f"Insufficient Stock! '{prod['name']}' has only {prod['stock']} left. You tried to give {gv_qty}.", "danger")
                        return redirect(url_for('morning'))
            ...
            for i, pid in enumerate(p_ids):
                ...
                if op_qty > 0 or gv_qty > 0:
                    ...
                    if gv_qty > 0:
                        cursor.execute("UPDATE products SET stock = stock - %s WHERE id = %s", (gv_qty, pid))
            ...
            if inserted_count > 0:
                ...
                flash("Warning: No items to allocate.", "warning")
...
def evening():
    ...
    if request.method == 'POST':
        try:
            print("--- Evening Submission Started ---") # Debug log
            ...
            if not emp_id or not date_val:
                flash("Error: Missing Employee or Date.", "danger")
                return redirect(url_for('evening'))

            # Time (Full Datetime for MySQL Timestamp)
            ist_now = datetime.utcnow() + timedelta(hours=5, minutes=30)
            time_str = ist_now.strftime('%Y-%m-%d %H:%M:%S') 
            ...
            if draft_id and draft_id.strip() != "":
                ...
                cursor.execute("""
                    ...
                    WHERE id=%s
                """, (total_amt, cash, online, discount, emp_c_amt, emp_c_note, emp_d_amt, emp_d_note, 
                ...
                if cursor.fetchone():
                    flash("Submission failed: Form for this date already exists.", "warning")
                    return redirect(url_for('evening'))

                # Insert New
                # Fix: Handle empty alloc_id safely
                final_alloc_id = alloc_id if (alloc_id and alloc_id.strip() != '' and alloc_id != '0') else None
                ...
                """, (final_alloc_id, emp_id, date_val, total_amt, cash, online, discount,
            ...
            for i, pid in enumerate(prod_ids):
                if not pid: continue
                # Safe Parsing
                p_tot = int(float(total_qtys[i])) if (i < len(total_qtys) and total_qtys[i]) else 0
                p_sold = int(float(sold_qtys[i])) if (i < len(sold_qtys) and sold_qtys[i]) else 0
                p_ret = int(float(return_qtys[i])) if (i < len(return_qtys) and return_qtys[i]) else 0
                p_price = float(prices[i]) if (i < len(prices) and prices[i]) else 0.0
                p_remain = max(0, p_tot - p_sold - p_ret)
                ...
                # INVENTORY: Add Returns BACK to Stock (Only on Final)
            ...
            if status == 'final':
                if emp_c_amt > 0:
                    desc = f"Evening Credit (Ref #{settle_id})" + (f": {emp_c_note}" if emp_c_note else "")
                    cursor.execute("""
                        INSERT INTO employee_transactions (employee_id, transaction_date, type, amount, description, created_at)
                        VALUES (%s, %s, 'credit', %s, %s, %s)
                    """, (emp_id, date_val, emp_c_amt, desc, time_str))

                if emp_d_amt > 0:
                    desc = f"Evening Debit (Ref #{settle_id})" + (f": {emp_d_note}" if emp_d_note else "")
                    cursor.execute("""
                        INSERT INTO employee_transactions (employee_id, transaction_date, type, amount, description, created_at)
                        VALUES (%s, %s, 'debit', %s, %s, %s)
                    """, (emp_id, date_val, emp_d_amt, desc, time_str))

            conn.commit()
            print("--- Evening Saved Successfully ---")
            
            if status == 'draft':
                flash("Draft Saved Successfully!", "info")
            else:
                flash("Evening Settlement Finalized!", "success")
                
            return redirect(url_for('evening'))

        except Exception as e:
            conn.rollback()
            print(f"Evening Save Error: {e}")
            flash(f"Error: {e}", "danger")
            return redirect(url_for('evening'))
...
def api_fetch_stock():
    ...
    try:
        ...
        if cur.fetchone():
            ...
            return jsonify({
                "mode": "locked",
                "evening_settled": True,
                "message": "Evening settlement is already finalized for this date."
            })

        # 2. Check for ANY Allocations Today
        cur.execute("""
            SELECT id FROM morning_allocations 
            WHERE employee_id=%s AND date=%s
        """, (employee_id, formatted_date))
        today_allocs = cur.fetchall() # Get ALL allocations
        ...
        if today_allocs:
            mode = "restock"
            # Get IDs of ALL today's allocations
            all_alloc_ids = [row['id'] for row in today_allocs]
            format_strings = ','.join(['%s'] * len(all_alloc_ids))
            
            # Aggregate items from ALL allocations
            query = f"""
                ...
                GROUP BY p.id
            """
            cur.execute(query, tuple(all_alloc_ids))
            ...
            # Normal Mode: Fetch Last Settlement
            cur.execute("""
                SELECT id 
                FROM evening_settle 
                WHERE employee_id = %s AND date < %s 
...
# --- ROUTE: API FETCH EVENING DATA (The Aggregator Fix) ---
...
def fetch_evening_data():
    ...
    try:
        ...
        # 1. CHECK EXISTING EVENING RECORD (Draft/Final)
        cur.execute("""
        ...
        if existing_record:
            ...
            # 2. CHECK ALL MORNING ALLOCATIONS (Morning + Restocks)
            # Find ALL allocation IDs for this employee on this date
            cur.execute("""
                SELECT id FROM morning_allocations 
                WHERE employee_id=%s AND date=%s 
            ...
            if today_allocs:
                source = "morning"
                # We use the FIRST allocation ID as the primary link, but calculate sums from ALL
                first_alloc_id = today_allocs[0]['id']
                all_alloc_ids = [row['id'] for row in today_allocs]
                
                # Create a placeholder string for SQL IN clause (e.g., %s, %s, %s)
                format_strings = ','.join(['%s'] * len(all_alloc_ids))
                
                # --- AGGREGATION QUERY ---
                # Logic: Total Qty = (Opening of FIRST Alloc) + (Sum of Given of ALL Allocs)
                # Why? Because Opening of Restock is just current-hand stock, not new stock from warehouse.
                # Only 'Given' adds new stock from warehouse.
                query = f"""
                    SELECT 
                        p.id as product_id, 
                        p.name, 
                        ...
                        -- Calculate Total:
                        (
                            -- 1. Get Opening from the FIRST allocation only (Start of Day)
                            COALESCE(
                                (SELECT opening_qty FROM morning_allocation_items 
                                 WHERE allocation_id = %s AND product_id = p.id LIMIT 1), 
                            0)
                            + 
                            -- 2. Add SUM of Given from ALL allocations (Morning + Restocks)
                            SUM(COALESCE(mai.given_qty, 0))
                        ) as total_qty,
                ...
                # Params: First ID (for subquery) + All IDs (for main query)
                params = [first_alloc_id] + all_alloc_ids
                cur.execute(query, tuple(params))
                items_list = cur.fetchall()
                
                # Link to the latest allocation ID for reference, or keep first. 
                # Keeping first usually maintains the day's chain better.
                ...
                if last_settle:
                    ...
                    return jsonify({'status': 'error', 'message': 'No stock found (No allocations today & no history).'})
